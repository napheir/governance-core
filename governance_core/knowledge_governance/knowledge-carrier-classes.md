...
| `reference` | 不漂移叙事 / 系统结构 / 概念图 | `knowledge/{governance,domain,methodology,research,...}/` | 否 | 否 |
| `runbook` | 操作步骤 / playbook | `knowledge/operations/*-manual.md`、`docs/*-manual.md` | 否 | 否 |
| `experiment-record` | frozen 实验报告（实验协议已强制冻结） | `knowledge/experiments/`（含 `archive/`）、`knowledge/datasets/` | 否 | 否 |
...
**示例**：`knowledge/decisions/adr-001-<topic>.md`、`adr-<decision-slug>.md`
...
**示例**：`knowledge/governance/scope-enforcement-mechanism.md`、
...
**示例**：`docs/core-manual.md`（gc 自身）、消费者域 `knowledge/operations/<agent>-manual.md`

### 2.4 `experiment-record`

**性质**：一次实验的 frozen 报告——配置 / 数据集 / 结果 / 结论。消费者域的实验协议
文档（如 `knowledge/methodology/<experiment-protocol>.md`）可强制要求这类报告写定
不动；本 class 把该约束显式化。
...
**示例**：`knowledge/experiments/EXP-<id>-*.md`、`knowledge/datasets/<dataset>_<date>.md`
...
**示例**：`knowledge/models/<model>_current.md`（消费者域 production 快照）
...
## 3. 推荐子目录 → carrier-class 映射

> 这是**推荐的**子树组织约定，并非断言以下目录都在本仓"现存"。一个项目只物化它
> 实际需要的子树（gc 自身随包提供 `knowledge/governance/`；消费者按需增设自己的域
> 子树并沿用本表的 class 约定）。
