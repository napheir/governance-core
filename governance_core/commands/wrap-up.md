...
- **Skill learned** (`.claude/skills/learned/<name>.md`) — 本 session 提取的工作流；继续 4a–4b
...
分类决定后再进入 4a–4b。若本阶段教训不属于"workflow"类（guide / CLAUDE.md / knowledge / memory / discard），4a–4b 的自动提取应跳过并在检查清单标明去向。

### 4a. 自动提取判断

```bash
python -m governance_core.discovery.tracker --should-extract
```
...
确认所有 skill 变更被 registry 正确发现（扫描本项目 `.claude/skills/` 的 guide / learned / command）：
```bash
python -m governance_core.discovery.registry --format table
```

## 4c. 候选上传（候选管道触发器，P-0072）
