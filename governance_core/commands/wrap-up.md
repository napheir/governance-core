...
**Invocation pattern**: skill-learning code lives in `agent-core/skills/discovery/`
but state (tracker data, learned skills) belongs to the invoking agent's own repo.
Use `PYTHONPATH` to import the shared code while keeping `CWD` in your own clone —
do **not** `cd` into core, or state will be written across repo boundaries.
(This is the `shared-code-per-agent-state` guide pattern.)

```bash
export PYTHONPATH="$(git rev-parse --show-toplevel)/../agent-core"
```
